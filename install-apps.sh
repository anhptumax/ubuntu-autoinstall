...
# Goi nen tang — luon cai truoc vi cac buoc khac can den.
readonly PREREQ_PACKAGES=(ca-certificates curl wget gpg apt-transport-https)
...
# install_apt_packages <ten buoc> <goi...>
install_apt_packages() {
  local label="$1"; shift
  log "Cai ${label} tu kho Ubuntu..."
  local pending=()
  local pkg
  for pkg in "$@"; do
...
# Ten buoc -> ham cai. Thu tu o day la thu tu chay.
readonly ALL_STEPS=(apt chrome telegram discord ibus-bamboo localsend rustdesk)
...
step_description() {
  case "$1" in
    ...
    ibus-bamboo) echo "Bo go tieng Viet ibus-bamboo" ;;
...
run_step() {
  case "$1" in
    apt)         install_apt_packages "ung dung + codec" "${APT_PACKAGES[@]}" ;;
    chrome)      install_chrome ;;
    telegram)    install_telegram ;;
    discord)     install_discord ;;
    ibus-bamboo) install_ibus_bamboo ;;
    localsend)   install_localsend ;;
    rustdesk)    install_rustdesk ;;
    *)           warn "Khong biet buoc '$1'"; return 1 ;;
  esac
}

usage() {
  cat <<USAGE
Cai cac phan mem can thiet cho Ubuntu Desktop.

  sudo ./install-apps.sh                 # cai tat ca
  sudo ./install-apps.sh chrome rustdesk # chi cai vai thu
  ./install-apps.sh --list               # xem danh sach
  ./install-apps.sh --help

Chay lai bao nhieu lan cung duoc: cai gi co roi thi bo qua.
Log day du: ${LOG_FILE}
...
main() {
  ...
  if [[ $# -gt 0 ]]; then
    local arg found step
    for arg in "$@"; do
      found=0
      for step in "${ALL_STEPS[@]}"; do
        [[ "${arg}" == "${step}" ]] && found=1 && break
      done
      if [[ ${found} -eq 0 ]]; then
        echo "Khong biet buoc '${arg}'. Xem: $0 --list" >&2
        exit 1
      fi
      steps+=("${arg}")
    done
  ...
  install_apt_packages "goi nen tang" "${PREREQ_PACKAGES[@]}" || fail_step "prereq"

  local step
  for step in "${steps[@]}"; do
    run_step "${step}" || fail_step "${step}"
  done
